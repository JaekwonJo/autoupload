...
DEFAULT_CONFIG = {
    ...
    "video_search_input_selector": "",
    "video_result_card_selector": "",
    "video_more_button_selector": "",
    "video_download_menu_selector": "",
    "video_quality_option_selector": "",
    "video_download_wait_seconds": 60,
...
class FlowVisionApp:
    ...
    def _video_download_quality(self):
        quality = str(self.cfg.get("video_download_quality", "720P") or "720P").strip().upper()
        if quality not in ("720P", "1080P", "4K"):
            return "720P"
        return quality

    def _video_search_input_candidates(self):
        cands = []
        cands.extend(self._normalize_candidate_list(self.cfg.get("video_search_input_selector", "")))
        cands.extend([
            "input[placeholder*='검색' i]",
            "input[aria-label*='검색' i]",
    ...
    def _video_result_card_candidates(self):
        cands = []
        cands.extend(self._normalize_candidate_list(self.cfg.get("video_result_card_selector", "")))
        cands.extend([
            "article",
            "[role='listitem']",
            "li",
    ...
    def _video_more_button_candidates(self):
        cands = []
        cands.extend(self._normalize_candidate_list(self.cfg.get("video_more_button_selector", "")))
        cands.extend([
    ...
    def _video_download_menu_candidates(self):
        cands = []
        cands.extend(self._normalize_candidate_list(self.cfg.get("video_download_menu_selector", "")))
        cands.extend([
    ...
    def _video_quality_option_candidates(self, quality):
        quality = str(quality or "720P").strip().upper()
        cands = []
        cands.extend(self._normalize_candidate_list(self.cfg.get("video_quality_option_selector", "")))
        cands.extend([
    ...
    def _sanitize_download_label(self, raw):
    ...
    def _build_previous_video_label(self, prev_index):
        if prev_index < 0:
            return ""
        if self.cfg.get("asset_loop_enabled") and 0 <= prev_index < len(self.asset_loop_items):
            tag = self._sanitize_download_label(self.asset_loop_items[prev_index].get("tag", ""))
            if tag:
                return tag
        if not (0 <= prev_index < len(self.prompts)):
            return f"P{prev_index + 1:03d}"

        prompt = str(self.prompts[prev_index] or "").strip()
        m = re.match(r"^\s*([A-Za-z]+[0-9]+)\b", prompt)
        if m:
            tag = self._sanitize_download_label(m.group(1))
            if tag:
                return tag

        compact = re.sub(r"[^A-Za-z0-9]", "", prompt)
        head3 = self._sanitize_download_label(compact[:3])
        if len(head3) >= 3:
            return head3

        return f"P{prev_index + 1:03d}"

    def _browser_download_dir(self):
        profile_path = self._resolve_profile_dir()
        pref_path = profile_path / "Default" / "Preferences"
        try:
            if pref_path.exists():
                data = json.loads(pref_path.read_text(encoding="utf-8"))
                default_dir = (((data or {}).get("download") or {}).get("default_directory") or "").strip()
                if default_dir:
                    p = Path(default_dir)
    ...
    def _next_available_path(self, path_obj):
        ...
        while n <= 9999:
            ...
            n += 1
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        return parent / f"{stem}_{stamp}{suffix}"

    def _perform_video_download(self, label, quality=None, dry_run=False, wait_sec=None):
        if not self.page:
            raise RuntimeError("브라우저 페이지가 없습니다.")
        quality = str(quality or self._video_download_quality()).strip().upper()
        if quality not in ("720P", "1080P", "4K"):
            quality = "720P"
        if wait_sec is None:
            wait_sec = int(self.cfg.get("video_download_wait_seconds", 60) or 60)
        wait_sec = max(10, min(120, int(wait_sec)))
        label = self._sanitize_download_label(label)
        if not label:
            raise RuntimeError("다운로드 라벨이 비어 있습니다.")
        ...
        search_input, search_sel = self._wait_best_locator(
            self._video_search_input_candidates(),
            timeout_sec=8,
            prefer_enabled=False,
        )
        if search_input is None:
            raise RuntimeError("영상 검색 입력칸을 찾지 못했습니다.")
        used["video_search_input_selector"] = search_sel or ""
        ...
        try:
            ...
            try:
                search_input.type(label, delay=random.randint(20, 50))
            except Exception as e:
                raise RuntimeError(f"영상 검색어 입력 실패: {e}")
        self.page.keyboard.press("Enter")
        self.actor.random_action_delay("영상 검색 결과 로딩 대기", 0.4, 1.2)
        ...
        while time.time() < deadline:
            card_loc, card_sel = self._resolve_best_locator(
                self._video_result_card_candidates(),
                timeout_ms=800,
                prefer_enabled=False,
            )
            if card_loc is not None:
                used["video_result_card_selector"] = card_sel or ""
                try:
                    self.actor.move_to_locator(card_loc, label=f"결과 카드({label})")
                except Exception:
                    try:
            ...
            more_loc, more_sel = self._resolve_best_locator(
                self._video_more_button_candidates(),
            ...
            if more_loc is not None:
                used["video_more_button_selector"] = more_sel or ""
                break
            time.sleep(0.5)
        ...
        if not self._click_with_actor_fallback(more_loc, "더보기 버튼"):
            raise RuntimeError("더보기 버튼 클릭에 실패했습니다.")

        download_loc, download_sel = self._wait_best_locator(
            self._video_download_menu_candidates(),
            timeout_sec=8,
            prefer_enabled=False,
        )
        if download_loc is None:
            download_loc, download_sel = self._resolve_text_locator_any_frame(["다운로드", "Download"], timeout_ms=1000)
        if download_loc is None:
            raise RuntimeError("다운로드 메뉴를 찾지 못했습니다.")
        used["video_download_menu_selector"] = download_sel or ""

        try:
            self.actor.move_to_locator(download_loc, label="다운로드 메뉴")
        except Exception:
            try:
                download_loc.hover(timeout=1200)
            except Exception:
                pass
        self.actor.random_action_delay("품질 메뉴 펼침 대기", 0.15, 0.6)

        quality_loc, quality_sel = self._wait_best_locator(
            self._video_quality_option_candidates(quality),
            timeout_sec=6,
            prefer_enabled=False,
        )
        if quality_loc is None:
            quality_loc, quality_sel = self._resolve_text_locator_any_frame([quality], timeout_ms=900)
        if quality_loc is None:
            raise RuntimeError(f"{quality} 품질 항목을 찾지 못했습니다.")
        used["video_quality_option_selector"] = quality_sel or ""

        if dry_run:
            return {"path": None, "used": used}

        try:
            with self.page.expect_download(timeout=15000) as dl_info:
                if not self._click_with_actor_fallback(quality_loc, f"{quality} 다운로드"):
        ...
        target_dir = self._browser_download_dir()
        safe_label = self._sanitize_download_label(label) or "VIDEO"
        target_path = self._next_available_path(target_dir / f"{safe_label}{suffix}")
        try:
            dl.save_as(str(target_path))
        except Exception as e:
            raise RuntimeError(f"다운로드 저장 실패: {e}")
        return {"path": str(target_path), "used": used}

    def _apply_video_used_selectors(self, used):
        if not isinstance(used, dict):
            return
        for key, var_name in (
            ("video_search_input_selector", "video_search_input_selector_var"),
            ("video_result_card_selector", "video_card_selector_var"),
            ("video_more_button_selector", "video_more_selector_var"),
            ("video_download_menu_selector", "video_download_menu_selector_var"),
            ("video_quality_option_selector", "video_quality_option_selector_var"),
        ):
    ...
    def _try_download_previous_video_for_current_index(self):
        if not self.cfg.get("video_download_enabled", False):
            return
        if self.index <= 0:
            return
        prev_idx = self.index - 1
        label = self._build_previous_video_label(prev_idx)
        if not label:
            label = f"P{prev_idx + 1:03d}"
        quality = self._video_download_quality()
        try:
            wait_sec = int(self.cfg.get("video_download_wait_seconds", 60) or 60)
        except Exception:
            wait_sec = 60
        self.update_status_label(f"⬇️ 이전 영상 다운로드 시도 ({label})", self.color_info)
        try:
            result = self._perform_video_download(label=label, quality=quality, dry_run=False, wait_sec=wait_sec)
            self._apply_video_used_selectors(result.get("used"))
            self.save_config()
            self.log(f"⬇️ 이전 영상 다운로드 성공: {label} ({quality})")
        except Exception as e:
            self.log(f"⚠️ 이전 영상 다운로드 실패({label}): {e}")
    ...
    def _build_ui(self):
        ...
        video_body, _set_video_open = self._create_collapsible_section(left_card, "영상 다운로드", opened=False)
        video_f = tk.Frame(video_body, bg=self.color_bg)
        video_f.pack(fill="x", pady=6)

        self.video_download_var = tk.BooleanVar(value=self.cfg.get("video_download_enabled", False))
        tk.Checkbutton(
            video_f,
            text="자동화 이후 이전 영상 다운로드 사용 (기본 OFF)",
            variable=self.video_download_var,
            command=self.on_option_toggle,
            bg=self.color_bg,
            font=("Malgun Gothic", 10),
            activebackground=self.color_bg,
        ...
        video_quality_f = tk.Frame(video_f, bg=self.color_bg)
        video_quality_f.pack(fill="x", pady=(6, 6))
        tk.Label(video_quality_f, text="품질", bg=self.color_bg, font=("Malgun Gothic", 9)).pack(side="left")
        self.video_quality_var = tk.StringVar(value=self.cfg.get("video_download_quality", "720P"))
        self.combo_video_quality = ttk.Combobox(
            video_quality_f,
        ...
        self.combo_video_quality.pack(side="left", padx=(6, 0))
        self.combo_video_quality.bind("<<ComboboxSelected>>", self.on_option_toggle)

        tk.Label(video_f, text="영상 검색 입력 selector(선택)", bg=self.color_bg, font=("Malgun Gothic", 9)).pack(anchor="w")
        self.video_search_input_selector_var = tk.StringVar(value=self.cfg.get("video_search_input_selector", ""))
        self.entry_video_search_input_selector = tk.Entry(
            video_f,
            textvariable=self.video_search_input_selector_var,
            bg="#FFFFFF",
            fg="black",
            font=("Consolas", 10),
        )
        self.entry_video_search_input_selector.pack(fill="x", ipady=3, pady=(2, 4))
        self.entry_video_search_input_selector.bind("<FocusOut>", self.on_option_toggle)

        tk.Label(video_f, text="결과 카드 selector(선택)", bg=self.color_bg, font=("Malgun Gothic", 9)).pack(anchor="w")
        self.video_card_selector_var = tk.StringVar(value=self.cfg.get("video_result_card_selector", ""))
        self.entry_video_card_selector = tk.Entry(
            video_f,
            textvariable=self.video_card_selector_var,
            bg="#FFFFFF",
            fg="black",
            font=("Consolas", 10),
        )
        self.entry_video_card_selector.pack(fill="x", ipady=3, pady=(2, 4))
        self.entry_video_card_selector.bind("<FocusOut>", self.on_option_toggle)

        tk.Label(video_f, text="더보기(...) 버튼 selector(선택)", bg=self.color_bg, font=("Malgun Gothic", 9)).pack(anchor="w")
        self.video_more_selector_var = tk.StringVar(value=self.cfg.get("video_more_button_selector", ""))
        self.entry_video_more_selector = tk.Entry(
            video_f,
            textvariable=self.video_more_selector_var,
            bg="#FFFFFF",
            fg="black",
            font=("Consolas", 10),
        )
        self.entry_video_more_selector.pack(fill="x", ipady=3, pady=(2, 4))
        self.entry_video_more_selector.bind("<FocusOut>", self.on_option_toggle)

        tk.Label(video_f, text="다운로드 메뉴 selector(선택)", bg=self.color_bg, font=("Malgun Gothic", 9)).pack(anchor="w")
        self.video_download_menu_selector_var = tk.StringVar(value=self.cfg.get("video_download_menu_selector", ""))
        self.entry_video_download_menu_selector = tk.Entry(
            video_f,
            textvariable=self.video_download_menu_selector_var,
            bg="#FFFFFF",
            fg="black",
            font=("Consolas", 10),
        )
        self.entry_video_download_menu_selector.pack(fill="x", ipady=3, pady=(2, 4))
        self.entry_video_download_menu_selector.bind("<FocusOut>", self.on_option_toggle)

        tk.Label(video_f, text="품질 항목 selector(선택)", bg=self.color_bg, font=("Malgun Gothic", 9)).pack(anchor="w")
        self.video_quality_option_selector_var = tk.StringVar(value=self.cfg.get("video_quality_option_selector", ""))
        self.entry_video_quality_option_selector = tk.Entry(
            video_f,
            textvariable=self.video_quality_option_selector_var,
            bg="#FFFFFF",
            fg="black",
            font=("Consolas", 10),
        )
        self.entry_video_quality_option_selector.pack(fill="x", ipady=3, pady=(2, 6))
        self.entry_video_quality_option_selector.bind("<FocusOut>", self.on_option_toggle)

        video_btn_f = tk.Frame(video_f, bg=self.color_bg)
        video_btn_f.pack(fill="x", pady=(2, 0))
        ttk.Button(video_btn_f, text="🔍 영상 다운로드 selector 자동찾기", command=self.on_auto_detect_video_download_selectors).pack(side="left")
        ttk.Button(video_btn_f, text="🧪 영상 다운로드 selector 테스트", command=self.on_test_video_download_selectors).pack(side="left", padx=6)
    ...
    def on_option_toggle(self, event=None):
        ...
        self.cfg["video_download_enabled"] = self.video_download_var.get() if hasattr(self, "video_download_var") else self.cfg.get("video_download_enabled", False)
        video_quality = self.video_quality_var.get().strip().upper() if hasattr(self, "video_quality_var") else str(self.cfg.get("video_download_quality", "720P"))
        if video_quality not in ("720P", "1080P", "4K"):
            video_quality = "720P"
        self.cfg["video_download_quality"] = video_quality
        self.cfg["video_search_input_selector"] = self.video_search_input_selector_var.get().strip() if hasattr(self, "video_search_input_selector_var") else self.cfg.get("video_search_input_selector", "")
        self.cfg["video_result_card_selector"] = self.video_card_selector_var.get().strip() if hasattr(self, "video_card_selector_var") else self.cfg.get("video_result_card_selector", "")
        self.cfg["video_more_button_selector"] = self.video_more_selector_var.get().strip() if hasattr(self, "video_more_selector_var") else self.cfg.get("video_more_button_selector", "")
        self.cfg["video_download_menu_selector"] = self.video_download_menu_selector_var.get().strip() if hasattr(self, "video_download_menu_selector_var") else self.cfg.get("video_download_menu_selector", "")
        self.cfg["video_quality_option_selector"] = self.video_quality_option_selector_var.get().strip() if hasattr(self, "video_quality_option_selector_var") else self.cfg.get("video_quality_option_selector", "")
        try:
            wait_sec = int(self.cfg.get("video_download_wait_seconds", 60))
        except Exception:
            wait_sec = 60
        self.cfg["video_download_wait_seconds"] = max(10, min(120, wait_sec))
    ...
    def on_auto_detect_video_download_selectors(self):
        if self.running:
            ...
            return
        self.on_option_toggle()
        self._auto_detect_video_download_selectors_worker()

    def _auto_detect_video_download_selectors_worker(self):
        try:
            self.update_status_label("🔍 영상 다운로드 selector 자동 탐색 중...", self.color_info)
            self._ensure_browser_session()
            self.actor.set_page(self.page)
            ...
            probe_label = "S01"
            probe_quality = self._video_download_quality()
            result = self._perform_video_download(
                label=probe_label,
            ...
            self._apply_video_used_selectors(used)
            self.save_config()
            self.log(
                "🔍 영상 다운로드 selector 자동탐색 결과 | "
                f"검색입력: {used.get('video_search_input_selector') or '미탐지'} | "
                f"결과카드: {used.get('video_result_card_selector') or '미탐지'} | "
                f"더보기: {used.get('video_more_button_selector') or '미탐지'} | "
                f"다운로드: {used.get('video_download_menu_selector') or '미탐지'} | "
                f"품질({probe_quality}): {used.get('video_quality_option_selector') or '미탐지'}"
            )
            if used.get("video_search_input_selector") and used.get("video_more_button_selector") and used.get("video_download_menu_selector"):
                self.update_status_label("✅ 영상 다운로드 selector 자동탐색 완료", self.color_success)
            else:
                self.update_status_label("⚠️ 영상 다운로드 selector 일부 미탐지", self.color_error)
        except Exception as e:
            self.log(f"❌ 영상 다운로드 selector 자동탐색 실패: {e}")
            self.update_status_label("❌ 영상 다운로드 selector 자동탐색 실패", self.color_error)
    ...
    def on_test_video_download_selectors(self):
        if self.running:
            ...
            return
        self.on_option_toggle()
        self._test_video_download_selectors_worker()

    def _test_video_download_selectors_worker(self):
        try:
            self.update_status_label("🧪 영상 다운로드 selector 테스트 중...", self.color_info)
            self._ensure_browser_session()
            self.actor.set_page(self.page)
            ...
            default_label = "S01"
            if self.cfg.get("asset_loop_enabled") and self.asset_loop_items:
                default_label = self.asset_loop_items[0].get("tag") or default_label
            elif self.prompts:
                default_label = self._build_previous_video_label(0) or default_label

            test_label = simpledialog.askstring(
                "영상 다운로드 테스트",
                "테스트할 검색 태그를 입력하세요. (예: S01)",
                initialvalue=default_label,
                parent=self.root,
            )
            if test_label is None:
                self.update_status_label("ℹ️ 영상 다운로드 테스트 취소됨", self.color_info)
                return
            test_label = self._sanitize_download_label(test_label)
            if not test_label:
                messagebox.showwarning("입력 오류", "테스트 태그가 비어 있습니다.")
                self.update_status_label("⚠️ 영상 다운로드 테스트 취소", self.color_error)
                return
            ...
            self.log(
                f"🧪 영상 다운로드 selector 테스트 성공 | 태그: {test_label} | "
                f"품질: {self._video_download_quality()} | 저장: {saved_path}"
            )
            self.update_status_label("✅ 영상 다운로드 selector 테스트 통과", self.color_success)
        except Exception as e:
            self.log(f"❌ 영상 다운로드 selector 테스트 실패: {e}")
            self.update_status_label("❌ 영상 다운로드 selector 테스트 실패", self.color_error)
    ...
    def _run_task(self):
        ...
        try:
            ...
            if self.cfg.get("video_download_enabled", False) and self.index > 0:
                self._try_download_previous_video_for_current_index()
